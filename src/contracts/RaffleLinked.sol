...
contract RaffleLinked is Chainlinked, Ownable {
    ...
    onlyOwner
  {
    ...
    onlyOwner
  {
    ChainlinkLib.Run memory run = newRun(PRICE_SPEC_ID, this, "fulfillRandomNumber(bytes32,uint256)");
    run.add("url", "https://qrng.anu.edu.au/API/jsonI.php?length=1&type=uint16");
    chainlinkRequest(run, LINK(1));
  }
  ...
  function fulfillRandomNumber(bytes32 _requestId, uint256 _randomNumber)
    public
    checkChainlinkFulfillment(_requestId)
  {
    emit RequestRandomNumberFulfilled(_requestId, _randomNumber);
    currentRandomNumber = _randomNumber;
  }
